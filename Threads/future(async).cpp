...
long long getDotProduct(vector<long long>& v, vector<long long>& w) {
	...
	auto future1 = async([&] {
		return inner_product(v.begin(), v.begin() + (vSize / 4), w.begin(), 0);
		});

	auto future2 = async([&] {
		return inner_product(v.begin() + (vSize / 4), v.begin() + (vSize / 2), w.begin() + (vSize / 4), 0);
		});

	auto future3 = async([&] {
		return inner_product(v.begin() + (vSize / 2), v.begin() + (vSize * 3 / 4), w.begin() + (vSize / 2), 0);
		});

	auto future4 = async([&] {
		return inner_product(v.begin() + (vSize * 3 / 4), v.end(), w.begin() + (vSize * 3 / 4), 0);
		});
